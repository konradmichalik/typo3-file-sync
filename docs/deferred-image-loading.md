...
The swap is injected as an external `<script type="module">` tag that carries no nonce. TYPO3's own default frontend content security policy restricts `script-src` to a nonce proxy, so enabling `fileSync.deferredLoading` also registers a `Configuration/ContentSecurityPolicies.php` mutation extending `script-src` with `'self'`, scoped to the frontend. This is host-agnostic by design, since a path scoped to the script's own public URL is not an option: CSP host-source grammar requires a host part, and that URL differs per installation (classic vs Composer) and carries a content hash under Composer mode. A site with a custom, stricter policy that removes `'self'` again after this mutation runs will still block the swap.
...
- A storage with deferred loading enabled needs a non-deferrable fallback handler, such as the placeholder image generator, configured alongside the remote one. Without it the render has nothing left to answer with and produces no file at all rather than a placeholder.
- Enabling `fileSync.deferredLoading` widens `script-src` to `'self'` for the whole frontend, not only for the swap module's own URL, since a CSP source expression cannot be scoped to a path without a host. Any other same-origin script becomes loadable under `script-src` as a side effect, on any site that otherwise relies on the nonce proxy to keep it to nonce-tagged scripts only.
